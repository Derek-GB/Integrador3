...
# FIX: radio de "golpe" para la detección manual de daño.
# Ajusta este número al tamaño real de tu sprite/CollisionShape2D.
...
func _ready():
	...
	# FIX: nos unimos a un grupo para que Lightning nos pueda encontrar
	# sin depender de get_overlapping_bodies() / señales de Area2D,
	# que no son confiables mientras el árbol está en pausa.
	add_to_group("storm_player")
...
func take_damage():
	...
	_blink_damage()
	# FIX: process_always = true (2do parámetro) para que el timer
	# SÍ avance aunque el árbol esté en pausa. Antes estaba en false
	# por error y el jugador se quedaba invisible para siempre.
	await get_tree().create_timer(DAMAGE_COOLDOWN, true).timeout
...
func _blink_damage():
	for index in range(BLINK_REPETITIONS):
		visible = false
		await get_tree().create_timer(BLINK_TIME, true).timeout
		visible = true
		await get_tree().create_timer(BLINK_TIME, true).timeout
